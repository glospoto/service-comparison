...
"""
This is the main framework's class. It has in charge the orchestration of the different operations 
of the framework.
"""
...
class ComparisonFramework(object):
	def __init__(self):
		...
		# The topology
		self._topology = None
		# Factory loader. For each alternative, a new environment is loaded accordingly with the alternative itself.
	...
	'''
	Initialize the framework, namely take arguments, create the topology and parse the 
	configuration file.
	'''
	...
	def run(self):
		...
		# Run simulation: for each service to evaluate, create a simulation and delegate to the 
		# loader objects the decision about the environment to load based on the alternatives
		services = self._parser.get_services()

		for service in services:
			# For each service, create the overlay. This overlay will be used for all alternatives 
			# of the given service
			self._log.debug(
				self.__class__.__name__, 'Creating overlay for service %s.', service.get_name())
			service.create_overlay(self._topology.get_topology())
			self._log.info(
				self.__class__.__name__, 
				'Overlay for service %s has been successfully created.', service.get_name())

			# Create the scenario for the service
			self._log.debug(
				self.__class__.__name__, 'Building scenario for service %s.', service.get_name())
			service.build_scenario()
			self._log.info(
				self.__class__.__name__, 
				'Scenario %s has been correctly built.', service.get_scenario())
			...
			for alternative in service.get_alternatives():
				...
				self._log.debug(
					self.__class__.__name__, 
					'Loading the environment for the alternative %s.', alternative)
				...
				self._log.info(
					self.__class__.__name__, 
					'Alternative %s for service %s has been successfully tested.', 
