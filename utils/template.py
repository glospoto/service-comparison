...
class ZebraTempate(Template):
	...
	def generate(self):
		...
		self._log.debug(
			self.__class__.__name__, 'Starting to load the template for Zebra configuration file.')
		zebra_template = self._template.get_template('zebra.mako')
		# Now save the file into a file inside tmp folder
		self._log.debug(
			self.__class__.__name__, 
			'Creating the file in which saving the Zebra configuration file.')
		...
		zebra_file.save()
		self._log.debug(
			self.__class__.__name__, 'Zebra configuration file has been successfully stored.')

		# Do the same for daemons file
		self._log.debug(
			self.__class__.__name__, 
			'Starting to load the template for Zebra daemons configuration file.')
		...
		self._log.debug(
			self.__class__.__name__, 
			'Zebra daemons configuration file has been successfully stored.')
...
class OspfTempate(Template):
	...
	def generate(self):
		...
		self._log.debug(
			self.__class__.__name__, 'Starting to load the template for OSPF configuration file.')
		ospf_template = self._template.get_template('ospfd.mako')
		# Now save the file into a file inside tmp folder
		self._log.debug(
			self.__class__.__name__,
			'Creating the file in which saving the OSPF configuration file.')
		...
		ospf_file.save()
		self._log.debug(
			self.__class__.__name__, 'OSPF configuration file has been successfully stored.')
...
class GoBgpTempate(Template):
	...
	def generate(self):
		...
		# GoBGP configuration
		self._log.debug(
			self.__class__.__name__, 'Starting to load the template for GoBGP configuration file.')
		gobgp_template = self._template.get_template('gobgp.mako')
		# Now save the file into a file inside tmp folder
		self._log.debug(
			self.__class__.__name__, 
			'Creating the file in which saving the GoBGP configuration file.')
		...
		gobgp_file.save()
		self._log.debug(
			self.__class__.__name__, 'GoBGP configuration file has been successfully stored.')
		"""
...
class BagPipeBgpTempate(Template):
	...
	def generate(self):
		...
		bagpipebgp_file.save()
		self._log.debug(
			self.__class__.__name__, 'BagPipeBGP configuration file has been successfully stored.')