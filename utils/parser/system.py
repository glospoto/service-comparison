...
"""
This class implements a parser for conf/system.xml file. That file contains all available 
alternatives, all association between alternatives and environments, namely in which environment an 
alternative can be execute and all available metrics.
"""
...
class SystemParser(object):
	def __init__(self):
		...
		self._services = {}
		# Mapping between services and alternatives. 
		# This is a map<service_name, list(alternative_name)>
		self._service_to_alternatives = {}
		# All alternatives. This is a map<name, alternative_adapter>
		self._alternatives = {}
		# All metrics. This is a list
		self._metrics = []
		# Mapping between metric and collector adapter.
		# This is a map<metric_name, collector_adapter_name>
		self._metric_to_collector = {}
		# Mapping between metric and extractor adapter.
		# This is a map<metric_name, extractor_adapter_name>
		self._metric_to_extractor = {}
		# All environments. This is a map<env_name, adapter>
		self._environments = {}
		# Mapping between environments and alternatives.
		# This is a map<env_name, list(alternative_name)>
	...
	def _parse_alternatives_for_service(self, service, service_name):
		self._log.info(
			self.__class__.__name__, 'Parsing alternatives for service %s.', service_name)
		...
		for alternative in alternatives:
			...
			self._log.debug(
				self.__class__.__name__, 
				'Alternative adapter %s for alternative %s has been loaded.', adapter, name)
	...
	def _parse_metrics(self, metrics):
		...
		for metric in metrics:
			...
			self._log.debug(
				self.__class__.__name__, 
				'Extractor adapter %s for metric %s has been loaded.', extractor_adapter, name)
			...
			if collector_adapter is not None:
				self._log.debug(
					self.__class__.__name__, 
					'Collector adapter %s for metric %s has been loaded.', collector_adapter, name)
				self._metric_to_collector[name] = collector_adapter
			else:
				self._log.debug(
					self.__class__.__name__, 'Metric %s has not a collector adapter.', name)
	...
	def _parse_environments(self, environments):
		...
		# For each environment, add entries into _environments and environment_to_alternatives' 
		# maps
		for environment in environments:
			...
			self._log.debug(
				self.__class__.__name__, 
				'Environment %s with adapter %s has been loaded.', name, adapter)
			...
			for mapping in mappings:
				...
				self._log.debug(
					self.__class__.__name__,
					'Alternatives %s are going to added to environment %s.', alternatives, name)
