...
class Parser(object):
	def __init__(self):
		...
		# All services expressed in the configuration file. Each service is an instance of 
	...
	def parse(self, config_file):
		...
		self._log.debug(
			self.__class__.__name__, 'Checking if config file %s exists.', configuration_file)
		if os.path.isfile(configuration_file):
			self._log.info(
				self.__class__.__name__, 'Loading configuration from %s.', configuration_file)
			# Take the absolute path of the configuration file
			self._parser = ConfigObj(infile=configuration_file)
		else:
			self._log.error(
				self.__class__.__name__,
				'File %s does not exist; please try again with a valid configuration file.', 
				config_file)
			sys.exit(-1)
		...
		'''
		Check if configuration file just contains one service or more. Here the problem is that 
		ConfigObj returns a string if the value of a key is just an element; it returns a list 
		otherwise. In the former case, transform it as a string
		'''
		...
		'''
		For each service, create a Service object with its alternatives and metrics. Here I'm sure 
		that the service is valid (it has been already checked).
		'''
		for service_name in services:
			...
			self._service_parser = self._factory_service_parser.create_service_parser(
				service_parser, service_name)
			self._log.info(
				self.__class__.__name__,
				'Specialized service parser %s has been successfully created.', 
			...
			'''
			# service_block is the piece of configuration file containing all stuff related to a 
			# service
			service_block = self._parser[service_name]
			# Take alternatives and metrics for this service
			self._log.debug(
				self.__class__.__name__, 'Loading alternatives for service %s.', service_name)
			alternatives = service_block['alternatives']
			# If the alternative is just one, the ConfigObj does not return a list, but a single 
			# object: transform it into a list
			if not isinstance(alternatives, list):
				alternatives = [alternatives]
			self._log.debug(
				self.__class__.__name__, 'Loading metrics for service %s.', service_name)
			...
			if not isinstance(metrics, list):
				metrics = [metrics]
			self._log.debug(
				self.__class__.__name__, 'Loading scenario for service %s.', service_name)
			'''
			Load scenario based on the section 'Scenario' of this service: s is an instance of 
			class Service; service is the "configuration block" of the configuration file from 
			which retrieve other configuration information (like alternatives) and alternatives is 
			the list of alternatives declared in the [Service] section in the configuration file.
			'''
			self._handle_alternative_and_metric_for_service(
				service, service_block, alternatives, metrics)
	...
	def _check_services(self, services):
		for service in services:
			if not self._system_parser.service_is_valid(service):
				self._log.error(
					self.__class__.__name__, 
					'Service %s is not a valid service; please check its name.', service)
				sys.exit(-1)

	'''
	Check if the association between alternative and environment specified as parameters is correct 
	in accord with the associations declared into the file system.xml. Actually, this method also 
	checks if alternatives and environments are valid in accord with the system.xml file.
	'''
	...
	def _check_mapping_alternative_to_environment(self, alternative, environment):
		...
		if self._system_parser.mapping_alternative_to_environment_is_valid(alternative, environment):
			self._log.error(
				self.__class__.__name__,
				'Alternative %s cannot be executed into environment %s; ' + 
				'please check your configuration file.', alternative, environment)
			sys.exit(-1)

	'''
	This method has in charge the task of creating alternative objects for the service passed as 
	parameter; moreover, on that alternative, it has also in charge the task of creating the 
	corresponding scenario
	'''

	def _handle_alternative_and_metric_for_service(
			self, service, service_block, alternatives, metrics):
		for alternative_name in alternatives:
			...
			alternative = service.create_alternative(
				alternative_name, alternative_adapter, alternative_parameters)
			self._log.info(
				self.__class__.__name__, 'Alternative %s has been created.', alternative_name)
	...
	def _handle_metrics_for_alternative(self, alternative, environment, metrics):
		for metric_name in metrics:
			...
			# Load the extractor and collector adapters. They are strings, because the extractor 
			# and collector are based both on the metric and the environment to use
			extractor_adapter = self._system_parser.get_extractor_adapter(metric_name)
			# Load the actual adapter, based on this metric and the environment for the selected 
			# alternative
			extractor_class = self._factory_extractor.create_extractor(
				extractor_adapter, environment)
			self._log.info(
				self.__class__.__name__, 
				'Extractor %s has been successfully created.', extractor_adapter)
			collector_adapter = self._system_parser.get_collector_adapter(metric_name)
			if collector_adapter is not None:
				collector_class = self._factory_collector.create_collector(
					collector_adapter, environment)
				self._log.info(
					self.__class__.__name__, 
					'Collector %s has been successfully created.', collector_adapter)
			else:
				collector_class = None
				self._log.info(
					self.__class__.__name__, 
					'Extractor %s does not need any collectors.', extractor_adapter)
			# Create the metric
			metric = Metric(metric_name, collector_class, extractor_class)
			self._log.info(
				self.__class__.__name__, 
				'Metric %s has been successfully created.', metric)
