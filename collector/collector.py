...
class FactoryCollector(object):
	...
	'''
	Starting from the name of collector class and the environment which this collector will run on, 
	it creates a collector object.
	'''

	def create_collector(self, collector_name, environment):
		self._log.info(
			self.__class__.__name__, 'Starting to create collector %s for environment %s', 
			collector_name, environment)
		'''
		Each extractor depends on the metric to extract and the environment in which the simulation 
		is running. In general, each environment has its own way for extracting data, so there will 
		be more implementation for the same metric. By convention, a class that models an extractor 
		for a specific environment is called as follow:	Environment+Metric. For example, to extract 
		device-load metric from mininet, the class will be called MininetDeviceLoad. So, for 
		instantiating the extractor, just concatenate the parameter environment to the last part of 
		extractor name
		'''
		...
		self._log.debug(
			self.__class__.__name__, 'Creating a new instance for %s', collector_class_name)
		# Load an instance
		self._collector = Class.for_name(collector_class_name)
		self._log.info(
			self.__class__.__name__, 
			'Collector %s has been successfully created.', collector_class_name)
		return self._collector


"""
Abstract class that model a collector. A Collector extends Thread: this implies that each collector 
will be executed in a separated thread with respect to the execution of the environment.
"""
