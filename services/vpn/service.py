...
"""
This class specializes class Service for VPN service. It has in charge the task of creating the 
alternatives under test for this service.
"""
...
class VpnService(Service):
	...
	def create_alternative(self, alternative_name, alternative_adapter, scenario_parameters):
		...
		alternative = Class.for_name(
			alternative_adapter, name=alternative_name, scenario=scenario_parameters)
		# Add the alternative to the list of alternatives
		self._alternatives.append(alternative)
		self._log.debug(
			self.__class__.__name__, 
			'Adding alternative %s to the list of alternatives of service %s.', 
			alternative_name, self._name)
		self._log.info(
			self.__class__.__name__, 
			'Alternative %s has been successfully created.', alternative_name)
		return alternative
	...
	def create_overlay(self, topology):
		...
		self._log.debug(
			self.__class__.__name__, 
			'Starting to create the overlay for the service %s', self._name)
		...
		for node in nodes:
			...
			self._log.debug(
				self.__class__.__name__,
				'Switch %s has been successfully created and added to %s.', 
				switch, self._overlay.get_name())

		# For each link in the topology, create a Link object in the VpnOverlay
		edges = topology.edges()
		for edge in edges:
			'''
			When getting switches from the overlay, remember to convert the id in the edge tuple 
			into an int value; moreover, plus 1 to it, avoiding dpid with value zero.
			'''
			...
			link = Link(
				from_switch, to_switch, from_switch_interface, to_switch_interface, subnet)
			self._overlay.add_link(link)
			self._log.debug(
				self.__class__.__name__,
				'Link %s has been successfully created and added to %s.',
				link, self._overlay.get_name())

		self._log.info(
			self.__class__.__name__, 
			'Overlay %s has been successfully created', self._overlay.get_name())
	...
	def create_scenario(self, scenario_params):
		self._log.debug(
			self.__class__.__name__, 'Starting to create the scenario for service %s.', self._name)
		self._scenario = VpnScenario(scenario_params)
		self._log.info(
			self.__class__.__name__,
			'Scenario for service %s has been successfully created.', self._name)
	...
	def build_scenario(self):
		self._log.debug(
			self.__class__.__name__, 'Starting to build the scenario for service %s.', self._name)
		self._scenario.create(self._overlay)
		self._log.debug(
			self.__class__.__name__,
			'Scenario for service %s has been successfully built.', self._name)
