...
"""
This class models a VirtualPrivateNetwork (VPN). A VPN consists of two PEs and two hosts, each of 
which is connected to a PE
"""
...
"""
This class models a site. Each VPN has at least two sites linked on different PEs. So, a site 
represents the VPN subnet linked at a certain PE at a well known port
"""
...
class Switch(object):
	...
	'''
	This method parse the name of the switch replacing undesirable characters (like ',', '_', 
	spaces, ...)
	'''
...
class Link(object):
	...
	def __repr__(self):
		return 'Link[from=%s:%s, to=%s:%s]' % (
			self._from_switch, self._from_switch_interface, self._to_switch, 
			self._to_switch_interface)

	'''
	Return the name of the link. That name is created by composing the name of the source node and 
