...
class Rm3SdnVpnAlternative(Alternative):
	...
	'''
	Deploy the alternative into the scenario associated to the service which this alternative 
	belongs to.
	'''
	...
	def deploy(self, overlay, scenario):
		...
		self._log.debug(
			self.__class__.__name__, 'Passing the configuration files to the controller.')
		...
		self._fs.copy(
			self._fs.join(
				self._fs.get_tmp_folder(), self._configurator.get_system_config_file()),
				self._fs.join(self._controller_path, 'conf'))
		# vpns.xml file will be moved into controller_path/conf/vpns folder
		self._fs.copy(
			self._fs.join(self._fs.get_tmp_folder(), self._configurator.get_vpns_config_file()),
		...
		self._log.info(
			self.__class__.__name__, 'Alternative %s has been successfully deployed.', self._name)
	...
	def destroy(self):
		...
		self._log.info(
			self.__class__.__name__, 'Alternative %s has been successfully stopped.', self._name)
